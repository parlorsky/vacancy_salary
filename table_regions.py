...
import json
import os
df = pd.DataFrame(columns = ['профессия','опыт работы']+[f'Регион_{i}' for i  in range(1,357)])
...
for i in prof:
    models = [x for x in os.listdir('/Users/parlorsky/Desktop/vacancy_salary/'+i) if 'model' in x and 'obl'  in x]
    for model in models:
        a = list(json.load(open('/Users/parlorsky/Desktop/vacancy_salary/'+i + '/' + model)).keys())
        
        stag = '0' if '0' in model else '1' if '1' in model else '2,3'
        vahta = '0' if len(models) == 3 else '1' if len(models) == 6 and 'y' in model else '0'
        if vahta == '0':
            mask = [d[i],stag] + [x for x in a]
            mask = mask + ['' for x in range(358-len(mask))]
            df.loc[len(df.index)] = mask
            
            

df.to_csv('ценность регионов по убыванию.csv',index = False,sep =';',encoding='utf-8-sig')
